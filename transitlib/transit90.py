...
class Transit90(object):
    ...
    def run(self):
        ...
        if self.orbit_radius is None:
            raise Exception("Invalid orbit radius!")

        if self.star_radius is None:
            raise Exception("Invalid star radius!")

        if self.planet_radius is None:
            raise Exception("Invalid planet radius!")

        if self.star_temperature is None:
            raise Exception("Invalid start temperature!")

        if self.planet_temperature is None:
            raise Exception("Invalid planet temperature!")

        if self.star_darkening is None:
            raise Exception("Invalid star darkening coeficient!")
